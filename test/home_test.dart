import 'package:alubank/components/box_card.dart';
import 'package:alubank/data/bank_http.mocks.dart';
import 'package:alubank/data/bank_inherited.dart';
import 'package:alubank/screens/home.dart';
import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mockito/mockito.dart';

void main() {
  final MockBankHttp httpMock = MockBankHttp();
  testWidgets("My widget has a text 'Saída' ", (tester) async {
    when(httpMock.dolarToReal()).thenAnswer((_) async => ('5'));
    await tester.pumpWidget(
      MaterialApp(
        home: BankInherited(
          child: Home(api: httpMock.dolarToReal()),
        ),
      ),
    );
    //verify(httpMock.dolarToReal()).called(1);
    final spentFinder = find.text('Saída');
    expect(spentFinder, findsWidgets);
  });
  testWidgets("Finds a LienearProgressIndicator", (tester) async {
    when(httpMock.dolarToReal()).thenAnswer((_) async => ('5'));
    await tester.pumpWidget(
      MaterialApp(
        home: BankInherited(
          child: Home(api: httpMock.dolarToReal()),
        ),
      ),
    );
    //verify(httpMock.dolarToReal()).called(1);
    expect(find.byType(LinearProgressIndicator), findsOneWidget);
  });
  testWidgets("Finds a a AccountStatus", (tester) async {
    when(httpMock.dolarToReal()).thenAnswer((_) async => ('5'));
    await tester.pumpWidget(
      MaterialApp(
        home: BankInherited(
          child: Home(api: httpMock.dolarToReal()),
        ),
      ),
    );
    //verify(httpMock.dolarToReal()).called(1);
    expect(find.byKey(const Key('recentActivity')), findsOneWidget);
  });
  testWidgets("Finds 5 BoxCards", (tester) async {
    when(httpMock.dolarToReal()).thenAnswer((_) async => ('5'));
    await tester.pumpWidget(
      MaterialApp(
        home: BankInherited(
          child: Home(api: httpMock.dolarToReal()),
        ),
      ),
    );
    //verify(httpMock.dolarToReal()).called(1);
    expect(
      find.byWidgetPredicate((widget) {
        if (widget is BoxCard) {
          return true;
        } else {
          return false;
        }
      }),
      findsNWidgets(5),
    );
  });
  testWidgets('When tap Deposit should upload earned in 10', (tester) async {
    when(httpMock.dolarToReal()).thenAnswer((_) async => ('5'));
    await tester.pumpWidget(
      MaterialApp(
        home: BankInherited(
          child: Home(api: httpMock.dolarToReal()),
        ),
      ),
    );
    //verify(httpMock.dolarToReal()).called(1);
    //* Tap para depositar 10
    await tester.tap(find.text('Depositar'));
    //* Tap para autailizar a tela para $10.0
    await tester.tap(find.text('Balanço disponível'));
    await tester.pumpAndSettle();
    expect(find.text('\$10.0'), findsOneWidget);
  });
  testWidgets("Testing MockHttp dolaToReal", (tester) async {
    when(httpMock.dolarToReal()).thenAnswer((_) async => ('5'));
    await tester.pumpWidget(
      MaterialApp(
        home: BankInherited(
          child: Home(api: httpMock.dolarToReal()),
        ),
      ),
    );
    verify(httpMock.dolarToReal()).called(6);
  });
}
